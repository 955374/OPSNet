...
import time
import pickle
from modules.voxel_utils.voxel.voxel_grid import VoxelGrid
...
class SiameseTrain(SiameseDataset):
    ...
    def getitem(self, index):
        ...
        sample_PC, sample_label, sample_reg, new_box_gt, align_gt_PC = utils.cropAndCenterPC_label(
            ...
            limit_area=self.area_extents)
        if sample_PC.nbr_points() <= 20:
            return self.getitem(np.random.randint(0, self.__len__()))

        sample_PC, sample_label, sample_reg, align_gt_PC, all_label = utils.regularizePCwithlabel(sample_PC,
                                                                                                  align_gt_PC,
                                                                                                  sample_label,
                                                                                                  sample_reg,
                                                                                                  self.input_size)
        # print(all_label.shape, sample_label.shape)
        ...
        gt_PC = utils.getModel([gt_PC_fir, gt_PC_pre], [gt_BB_fir, gt_BB_pre], offset=self.offset_BB, scale=self.scale_BB)
        ...
        if hm[0, corners_int_ul[1]:corners_int_br[1] + 1, corners_int_ul[0]:corners_int_br[0] + 1].shape != (
        corners_int_br[1] + 1 - corners_int_ul[1], corners_int_br[0] + 1 - corners_int_ul[0]):
            print(hm_grid.shape)
            return self.getitem(np.random.randint(0, self.__len__()))
        ...
        hm[0, [ct_image_int[1], ct_image_int[1], ct_image_int[1] + 1, ct_image_int[1] - 1], [ct_image_int[0] - 1,
                                                                                             ct_image_int[0] + 1,
                                                                                             ct_image_int[0],
                                                                                             ct_image_int[0]]] = 0.8
        ...
        for i in range(-self.regress_radius, self.regress_radius + 1):
            for j in range(-self.regress_radius, self.regress_radius + 1):
                ...
                loc_offsets[
                    (i + self.regress_radius) * (2 * self.regress_radius + 1) + (j + self.regress_radius)] = offsets
                ind_int = ct_image_int + np.array([i, j])
                ind_offsets[(i + self.regress_radius) * (2 * self.regress_radius + 1) + (j + self.regress_radius)] = \
                ind_int[1] * output_w + ind_int[0]
